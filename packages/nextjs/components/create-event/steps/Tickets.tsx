...
export const Tickets = ({ formData, setFormData, onNext, onBack }: StepProps) => {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      {/* Age Restriction */}
      <div className="form-control animate-slide-up" style={{ animationDelay: "0.25s" }}>
        <label className="label cursor-pointer justify-start gap-4">
          <span className="label-text font-bold text-neutral opacity-85">Age Restricted (18+ Only)</span>
          <input
            type="checkbox"
            className="toggle toggle-error"
            checked={formData.ageRestriction === 1}
            onChange={(e) => setFormData({ ...formData, ageRestriction: e.target.checked ? 1 : 0 })}
          />
        </label>
        <p className="text-xs text-neutral-500 mt-1">Attendees will need to generate a ZK Proof of Age to buy tickets.</p>
      </div>
